 ...
 * and the output of the program prints the equivalent ASCII character after segregating
	...
	for( i = 0; i < (sizeof(input)/sizeof(char)); i++)
	{
		...
		{	
			...
			{	
				...
				//If the input character is not special, keep the check false to execute else
				check = false;   
			}
		}
